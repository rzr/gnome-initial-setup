...
#define ADD_PAGE(pages, NAME) \
  ...
  page_data->page_id = gis_ ## NAME ## _page_get_id (); \
...
should_skip_page (const gchar *page_id, gchar **skip_pages)
{
  ...
  if (skip_pages) {
      while (skip_pages[i]) {
          g_debug ("skip_pages compareing %s %s", skip_pages[i], page_id);
  ...
  if (session_setup_mode) {
      ...
      while (i < G_N_ELEMENTS (system_setup_pages)) {
          g_debug ("compareing %s %s", system_setup_pages[i], page_id);
...
pages_to_skip_from_file ()
{
  ...
  if (g_key_file_load_from_file (skip_pages_file, "/tmp/skip_pages_file",
      ...
      g_debug ("skip pages %p", skip_pages);
      return skip_pages;
  }
...
rebuild_pages_cb (GisDriver *driver, GList *pages)
{
  ...
  for (pages_itr = pages;
       pages_itr != NULL;
       pages_itr = pages_itr->next) {

      PageData *page_data = pages_itr->data;

      if (should_skip_page (page_data->page_id, skip_pages) == FALSE)
